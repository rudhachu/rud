...
async function initialize() {
  ...
  // Check if the session directory and file exist
  if (!fs.existsSync(sessionPath)) {
    if (config.SESSION_ID) {
      ...
      try {
        ...
        // Write the downloaded file content to the specified path
        fs.writeFileSync(sessionPath, response.data);
        console.log("Session download completed!");
      } catch (error) {
        console.error("Failed to download the session file:", error.message);
      }
    } else {
      console.error("SESSION_ID is not provided in the configuration.");
    }
  } else {
