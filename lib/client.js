...
commands.map(async (command) => {
  ...
  // Check conditions for the command
  ...
  try {
    // Mark message as read if required
    ...
    // Handle "delete" specific command
    if (command.on === "delete" && message.type === "protocolMessage") {
      message.messageId = message.msg?.protocolMessage?.key?.id;
      await command.function(message, message.text, client);
      return;
    }
