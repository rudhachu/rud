...
const handleGroupMessage = async (msg, client) => {
   ...
   if (antilink?.enabled && msg.body) {
      ...
      if (urlPattern.test(msg.body)) {
         const isAdmin = (await client.groupMetadata(msg.from)).participants.find(p => p.id === msg.sender)?.admin;
         if (!isAdmin) {
            if (antilink.action === "kick") {
               await client.groupParticipantsUpdate(msg.from, [msg.sender], "remove");
            } else if (antilink.action === "warn") {
               await client.sendMessage(msg.from, { text: `@${msg.sender.split("@")[0]}, sending links is not allowed in this group.`, mentions: [msg.sender] });
            }
            await client.sendMessage(msg.from, { delete: msg.key });
         } else {
            await client.sendMessage(msg.from, { text: "Warning: As an admin, please be cautious when sharing URLs." });
         }
      }
   }
